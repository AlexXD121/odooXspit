...
import { Search, List, LayoutGrid, Plus, X, Check, Package } from "lucide-react";
import { createInboundOperation, validateOperation } from "../../actions";
...
export default function ReceiptClient({ initialReceipts, products }: { initialReceipts: Receipt[], products: Product[] }) {
    ...
    async function handleCreate(validate: boolean) {
        ...
        const res = await createInboundOperation(selectedItems);
                ...
                <div className="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">
                            {viewMode === "list" ? "Manage inbound receipts" : "Create new receipt"}
                        </p>
                    </div>
                    <button
                ...
                <>
                    ...
                    <div className="md:hidden space-y-4">
                        {initialReceipts.map((receipt) => (
                            <div key={receipt.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                <div className="flex justify-between items-start mb-2">
                                    <div>
                                        <span className="text-sm font-medium text-slate-900 block">{receipt.reference}</span>
                                        <span className="text-xs text-slate-500">{new Date(receipt.updatedAt).toLocaleDateString()}</span>
                                    </div>
                                    <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${receipt.status === 'DONE' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
                                        }`}>
                                        {receipt.status}
                                    </span>
                                </div>
                        ...
                        {initialReceipts.length === 0 && (
                            <div className="text-center py-12 bg-white rounded-xl border border-slate-200">
                                ...
                                <h3 className="text-slate-900 font-medium mb-1">No receipts found</h3>
                                <button
                                ...
                                <tbody className="divide-y divide-slate-200">
                                    {initialReceipts.map((receipt) => (
                                        <tr key={receipt.id} className="hover:bg-slate-50 transition-colors">
                                            <td className="px-6 py-4 text-sm font-medium text-slate-900">{receipt.reference}</td>
                                            <td className="px-6 py-4 text-sm text-slate-600">{new Date(receipt.updatedAt).toLocaleDateString()}</td>
                                            <td className="px-6 py-4">
                                                <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${receipt.status === 'DONE' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
                                                    }`}>
                                                    {receipt.status}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 text-sm text-slate-600">{receipt.moves.length} items</td>
                                        </tr>
                                    ))}
                                    {initialReceipts.length === 0 && (
                                        <tr>
                                            <td colSpan={4} className="px-6 py-12 text-center">
                                                ...
                                                <h3 className="text-slate-900 font-medium mb-1">No receipts found</h3>
                                                <p className="text-slate-500 text-sm mb-6">Create your first inbound receipt to get started.</p>
                                                <button
                                                    onClick={() => setViewMode("form")}
